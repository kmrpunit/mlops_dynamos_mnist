...
class RandomForestModel(BaseModel):
    ...
    def __init__(self, input_shape, name="rf"):
        super().__init__(name, input_shape)
        print("RandomForestModel: __init__")
        self.default_param_grid = { 
                'n_estimators': [50, 100, 150],
                'max_depth': [None, 10, 20],
                'min_samples_split': [2, 5, 10]
            }
        self.rfc_model = RandomForestClassifier(criterion='entropy', random_state=42)

    def train(self, X_train, y_train, param_grid, n_jobs, cv):
        print(f"RandomForestModel ({self._name}): train with param_grid: {param_grid} and n_jobs: {n_jobs} and cv: {cv}")

        # Create GridSearchCV with error_score='raise'
        grid = GridSearchCV(estimator=self.rfc_model, param_grid=param_grid, n_jobs=n_jobs, cv=cv, error_score='raise', verbose=3, return_train_score=True)
    ...
    def evaluate_model(self, grid_result, X_test, y_test):
        ...
        print(f"RandomForestModel ({self._name}): evaluate_model")
    ...
    def train_and_evaluate(self, X_train, y_train, X_test, y_test, param_grid, n_jobs, cv):
        print("RandomForestModel: train_and_evaluate")
